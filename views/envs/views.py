...
def get_all_envs():
    ...
    if not namespace_id:
        return jsonify({"code": 400, "message": "未指定命名空间"}), 400
    ...
    envs = envs_db.envs.find({"namespace_id": namespace_id}, {"name": 1, "nick_name": 1}).sort(sort_type, -1).skip(int(skip)).limit(int(current_max_row))  # 只返回名称和昵称
    for env in envs:
